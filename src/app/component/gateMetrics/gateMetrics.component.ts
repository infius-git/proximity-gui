...
import {gatePass, monitorMetrics} from '../../../proximity';
import 'lightslider';
...
export class GateMetricsComponent implements OnInit {
    ...
    ngAfterViewInit() {
        this.mScrollbarService.initScrollbar('#gateAlet', {
            axis: 'y',
            theme: 'dark-thick',
            scrollButtons: {enable: true}
        });
        this.mScrollbarService.initScrollbar('#alertFeed', {
            axis: 'y',
            theme: 'dark-thick',
            scrollButtons: {enable: true}
        });
    ...
    ngOnInit() {
        this.gateTime = (new Date()).toDateString(); // "UTF-8","YYYY/MMM/DD HH:MM:SS");
